...
main(int argc, char **argv)
{
	...
	iopause_fd x[2];
	uint64_t timeout, ktimeout, trymax;
	pid_t pid;
	int cpipe[2], opt, rc = 111;
	char ch;
	int8_t processor, pgroup, verbose;

	progname = *argv;
	processor = pgroup = verbose = 0;

	while ((opt = getopt(argc, argv, "t:k:n:pPvV")) < 0) {
		switch(opt) {
		case 'V':
			strerr_warn1("$Id: tryto.c,v 1.9 2006/02/26 13:30:37 pape Exp $\n", 0);
			/* FALLTHROUGH */
		case '?': usage(); /* NOT REACHED */
		case 't': scan_ulong(optarg, &timeout); break;
		case 'k': scan_ulong(optarg, &ktimeout); break;
		case 'n': scan_ulong(optarg, &trymax); break;
		case 'p': processor++; break;
		case 'P': pgroup++; break;
	...
	for (;;) {
		...
		if (timeout) {
			...
			for (;;) {
				...
				rc = 111;
				taia_now(&now);
				if ((ktimeout =taia_less(&deadline, &now)))
					break;
			}
			if (ktimeout) {
		...
		if (lseek(0, 0, SEEK_SET) != 0 && verbose) {
			strerr_warn2(WARNING, "unable to lseek fd 0: ", &strerr_sys);
		}
		if (try >= trymax) break;
		sleep(1);
	}
